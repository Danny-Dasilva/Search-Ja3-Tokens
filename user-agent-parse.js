...
let filtered_array = json_obj.filter(d => d.Count > 10);
filtered_array = filtered_array.filter(d => d["User-Agent"] !== '');
// console.log(arr1)
...
const newFilename = `user_agent-parsed.json` // name of a new file to be saved
await writeJSON(newFilename, filtered_array) // create a new JSON file with just the Bitcoin price
console.log("Wrote a post process file")

// Optionally delete the original file
await removeFile('./user_agent.json') // equivalent to removeFile('btc-price.json')